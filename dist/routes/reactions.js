...
const createRouter = (ctx) => {
    ...
    router.post('/', (0, http_1.handler)(async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                const collection = subjectType === 'review'
                ...
                subjectCid = subjectRecord.data.cid;
            }
            catch (err) {
                ctx.logger.warn({ err, subjectUri }, 'Could not fetch subject CID, using placeholder');
            }
            const record = {
                $type: 'app.collectivesocial.feed.react',
                emoji: emoji,
                subject: subjectType === 'review'
                    ? {
    ...
    router.get('/:subjectType/:encodedUri', (0, http_1.handler)(async (req, res) => {
    ...
    router.get('/user/:subjectType/:encodedUri', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
