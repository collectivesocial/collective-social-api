...
const createRouter = (ctx, app) => {
    ...
    app.get('/tags/search', async (req, res) => {
        try {
            ...
            const tags = await ctx.db
                ...
                'tags.slug',
                'tags.status',
                (0, kysely_1.sql) `COUNT(DISTINCT media_item_tags.media_item_id)`.as('usageCount'),
            ])
                ...
                .orderBy((0, kysely_1.sql) `COUNT(DISTINCT media_item_tags.media_item_id)`, 'desc')
                .limit(10)
                .execute();
            res.json({ tags });
        }
    ...
    app.get('/tags/:tagSlug/items', async (req, res) => {
        try {
            ...
            let query = ctx.db
                ...
                .innerJoin('media_items', 'media_item_tags.media_item_id', 'media_items.id')
                .leftJoin('tag_reports', (join) => join
            ...
            if (sort === 'relevant') {
                query = query
                    .select((0, kysely_1.sql) `COUNT(DISTINCT media_item_tags.user_did)`.as('tagCount'))
                    .groupBy([
                    'media_items.id',
                    'media_items.mediaType',
                    'media_items.title',
                    ...
                    .orderBy((0, kysely_1.sql) `COUNT(DISTINCT media_item_tags.user_did)`, 'desc');
            }
            else if (sort === 'rating') {
                query = query
                    .groupBy([
    ...
    app.get('/media/:itemId/tags', async (req, res) => {
        try {
            ...
            const tags = await ctx.db
                ...
                'tags.slug',
                (0, kysely_1.sql) `COUNT(DISTINCT media_item_tags.media_item_id)`.as('usageCount'),
                (0, kysely_1.sql) `BOOL_OR(users."isAdmin")`.as('hasAdminTag'),
            ])
    ...
    app.post('/media/:itemId/tags', async (req, res) => {
        try {
            ...
            await ctx.db
                ...
                .execute();
            ctx.logger.info({ itemId, tagId: tag.id, userDid: agent.did }, 'Added tag to media item');
            // Get usage count
            const usageCount = await ctx.db
                .selectFrom('media_item_tags')
                .select((0, kysely_1.sql) `COUNT(DISTINCT media_item_id)`.as('count'))
                .where('tag_id', '=', tag.id)
                .executeTakeFirst();
            res.json({
                tag: {
    ...
    app.delete('/media/:itemId/tags/:tagId', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
            ...
            ctx.logger.info({ itemId, tagId, userDid: agent.did }, 'Removed tag from media item');
    ...
    app.get('/admin/tags', async (req, res) => {
        try {
            ...
            const tags = await ctx.db
                ...
                'tags.status',
                'tags.created_at',
                (0, kysely_1.sql) `COUNT(DISTINCT media_item_tags.media_item_id)`.as('itemCount'),
                (0, kysely_1.sql) `COUNT(DISTINCT media_item_tags.user_did)`.as('userCount'),
            ])
                .groupBy([
                'tags.id',
                'tags.name',
    ...
    app.get('/admin/tags/merge-preview', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
            ...
            const affectedItems = await ctx.db
                ...
                .where('media_item_tags.tag_id', '=', parseInt(sourceTagId))
                .execute();
            // Check for duplicates (same item + user with target tag)
            const duplicates = await ctx.db
                .selectFrom('media_item_tags as source')
                .innerJoin('media_item_tags as target', (join) => join
                ...
                .where('source.tag_id', '=', parseInt(sourceTagId))
                .where('target.tag_id', '=', parseInt(targetTagId))
                .execute();
            res.json({
                affectedItems,
    ...
    app.put('/admin/tags/:tagId', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
            ...
            await ctx.db
                ...
                .where('id', '=', parseInt(tagId))
                .execute();
            ctx.logger.info({ tagId, oldName: existingTag.name, newName: name }, 'Tag updated');
    ...
    app.post('/admin/tags/merge', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
            ...
            await ctx.db
                ...
                eb.exists(eb
                    ...
                    .whereRef('target.media_item_id', '=', 'media_item_tags.media_item_id')
    ...
    app.post('/media/:itemId/tags/:tagId/report', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
            ...
            await ctx.db
                ...
                .execute();
            ctx.logger.info({ itemId, tagId, reporterDid: agent.did, reason }, 'Tag reported');
            res.json({ success: true });
        }
        catch (err) {
    ...
    app.get('/admin/tag-reports', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
            ...
            const reportCounts = await ctx.db
                .selectFrom('tag_reports')
                .select([
                'tag_reports.item_id',
                'tag_reports.tag_id',
                (0, kysely_1.sql) `COUNT(*)`.as('report_count'),
            ])
    ...
    app.post('/admin/tag-reports/:reportId/remove-tag', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
    ...
    app.post('/admin/tag-reports/:reportId/dismiss', async (req, res) => {
        try {
            const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
            if (!agent) {
                return res.status(401).json({ error: 'Not authenticated' });
            ...
            await ctx.db
                ...
                .where('id', '=', parseInt(reportId))
                .execute();
            ctx.logger.info({ reportId, adminDid: agent.did }, 'Tag report dismissed');
            res.json({ success: true });
        }
        catch (err) {
