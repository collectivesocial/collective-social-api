...
const createRouter = (ctx) => {
    ...
    router.get('/me', (0, http_1.handler)(async (req, res) => {
    ...
    router.get('/following/:did', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            const followRecord = response.data.records.find((record) => record.value.subject === did);
            res.json({
    ...
    router.post('/follow/:did', (0, http_1.handler)(async (req, res) => {
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        try {
            ...
            const existingFollow = listResponse.data.records.find((record) => record.value.subject === did);
            if (existingFollow) {
                return res.json({
                    success: true,
    ...
    router.delete('/unfollow/:did', (0, http_1.handler)(async (req, res) => {
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        try {
            ...
            const followRecord = listResponse.data.records.find((record) => record.value.subject === did);
            if (!followRecord) {
    ...
    router.delete('/me', (0, http_1.handler)(async (req, res) => {
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
