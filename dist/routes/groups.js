...
const createRouter = (ctx) => {
    ...
    router.get('/', (0, http_1.handler)(async (req, res) => {
        ...
        try {
            ...
            const memberCommunityDids = new Set();
            if (agent) {
                try {
     ...
     */
    router.get('/:did/permissions', (0, http_1.handler)(async (req, res) => {
        const { did } = req.params;
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        const userDid = agent?.did ?? undefined;
        try {
            const permissions = await (0, opensocial_1.resolveUserPermissions)(did, userDid);
    ...
    router.get('/:did', (0, http_1.handler)(async (req, res) => {
        const { did } = req.params;
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        const userDid = agent?.did ?? undefined;
        try {
            ...
            const inProgressItemUris = new Set(allStatuses
                .filter((s) => s.value.status === 'in-progress')
                .map((s) => s.value.listItemUri));
            const inProgressItems = allItems
                .filter((r) => inProgressItemUris.has(r.uri))
                .map((r) => ({ uri: r.uri, rkey: (0, opensocial_1.rkeyFromUri)(r.uri), ...r.value, status: 'in-progress' }));
            // Get member count
            let memberCount = 0;
            try {
            ...
            let permissions = {};
            try {
                permissions = await (0, opensocial_1.resolveUserPermissions)(did, userDid);
            }
            catch (permErr) {
    ...
    router.post('/:did/join', (0, http_1.handler)(async (req, res) => {
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        ...
        try {
            ...
            const joinInfo = await opensocial.joinCommunity(did, agent.did, 
    ...
    router.delete('/:did', (0, http_1.handler)(async (req, res) => {
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
