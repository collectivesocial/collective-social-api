...
const createRouter = (ctx) => {
    ...
    router.get('/recent', (0, http_1.handler)(async (req, res) => {
    ...
    router.post('/search', (0, http_1.handler)(async (req, res) => {
        ...
        try {
            ...
            if (mediaType === 'book') {
                ...
                const searchResponse = await (0, openlibrary_1.searchBooks)(query, limit, offset);
                // For each result, check if it exists in our database
                enrichedResults = (await Promise.all(searchResponse.results.map(async (result) => {
                    const isbn = (0, openlibrary_1.extractISBN)(result);
                    ...
                    return {
                        ...
                        coverImage: result.cover_i
                            ? (0, openlibrary_1.getCoverUrl)(result.cover_i, 'M')
                            : null,
                ...
                const searchResponse = await (0, omdb_1.searchOMDB)(query, omdbType, limit);
                // For each result, check if it exists in our database
                enrichedResults = await Promise.all(searchResponse.results.map(async (result) => {
                    let dbItem = null;
    ...
    router.post('/link', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await getSessionAgent(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        const normalizedUrl = (0, urlMetadata_1.normalizeUrl)(url);
        ...
        const metadata = await (0, urlMetadata_1.fetchUrlMetadata)(url);
        if (!metadata.title) {
            return res
                .status(400)
        ...
        const detectedType = mediaType || (0, urlMetadata_1.detectMediaTypeFromUrl)(url);
    ...
    router.post('/add', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await getSessionAgent(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        const { title, creator, mediaType, isbn, coverImage, publishYear, url, length, imdbId, } = req.body;
        if (!title || !mediaType) {
            return res
                .status(400)
        ...
        try {
            ...
            if (isbn && mediaType === 'book') {
                const bookDetails = await (0, openlibrary_1.getBookByISBN)(isbn);
                if (bookDetails) {
                    description = (0, openlibrary_1.extractDescription)(bookDetails);
            ...
            if (imdbId && (mediaType === 'movie' || mediaType === 'tv')) {
                const { getOMDBDetails, getTotalEpisodes } = await Promise.resolve().then(() => __importStar(require('../services/omdb')));
                ...
                if (details) {
                    ...
                    if (mediaType === 'movie' && !runtime && details.Runtime) {
                        runtime = (0, omdb_1.extractRuntime)(details.Runtime);
                    }
    ...
    router.get('/:id', (0, http_1.handler)(async (req, res) => {
        const { id } = req.params;
        try {
    ...
    router.get('/:id/reviews', (0, http_1.handler)(async (req, res) => {
        const { id } = req.params;
        ...
        try {
            ...
            const reviewsWithProfiles = await Promise.all(reviews.map(async (review) => {
                try {
                    const response = await fetch(`https://public.api.bsky.app/xrpc/app.bsky.actor.getProfile?actor=${review.authorDid}`);
                    ...
                    ctx.logger.error({ err, did: review.authorDid }, 'Failed to fetch profile for review author');
                    return {
                        id: review.id,
                        authorDid: review.authorDid,
    ...
    router.put('/:id', (0, http_1.handler)(async (req, res) => {
        ...
        const { title, creator, coverImage, description, publishedYear, length } = req.body;
        try {
            ...
            if (creator !== undefined)
                updateData.creator = creator || null;
    ...
    router.delete('/:id', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await getSessionAgent(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
