...
const createRouter = (ctx) => {
    ...
    router.get('/:mediaItemId', (0, http_1.handler)(async (req, res) => {
        ...
        try {
            ...
            completions.sort((a, b) => new Date(b.completedAt).getTime() -
    ...
    router.delete('/:completionUri', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
    ...
    router.get('/user/:did/:mediaItemId', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
