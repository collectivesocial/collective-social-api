...
const createRouter = (ctx) => {
    ...
    router.get('/', (0, http_1.handler)(async (req, res) => {
        ...
        try {
            ...
            const itemCounts = {};
            itemsResponse.data.records.forEach((record) => {
            ...
            res.json({
                collections: response.data.records.map((record) => ({
                    uri: record.uri,
                    cid: record.cid,
    ...
    router.post('/', (0, http_1.handler)(async (req, res) => {
        ...
        const { name, description, purpose, visibility, parentListUri } = req.body;
        if (!name) {
        ...
        try {
            ...
            const hasDefaultList = existingLists.data.records.some((record) => record.value.isDefault === true);
    ...
    router.put('/:listUri', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            const listRecord = listsResponse.data.records.find((record) => record.uri === listUri);
            if (!listRecord) {
                return res.status(404).json({ error: 'List not found' });
            }
            const currentData = listRecord.value;
    ...
    router.delete('/:listUri', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            const listData = listRecord.value;
    ...
    router.post('/:listUri/clone', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            const sourceList = listsResponse.data.records.find((record) => record.uri === sourceListUri);
            if (!sourceList) {
                return res.status(404).json({ error: 'Source list not found' });
            }
            const sourceListData = sourceList.value;
            ...
            const newListRecord = {
                ...
                purpose: sourceListData.purpose || 'app.collectivesocial.defs#curatelist',
                isDefault: false,
                createdAt: new Date().toISOString(),
            };
            ...
            const sourceItems = itemsResponse.data.records.filter((record) => record.value.list === sourceListUri);
            // Clone each item to the new list
            for (const sourceItem of sourceItems) {
                const sourceItemData = sourceItem.value;
                ...
                const newItemRecord = {
                    ...
                    order: sourceItemData.order !== undefined
                ...
                await agent.api.com.atproto.repo.createRecord({
                    repo: agent.did,
                    collection: 'app.collectivesocial.feed.listitem',
                    record: newItemRecord,
                });
            }
            ctx.logger.info({ sourceListUri, newListUri, itemCount: sourceItems.length }, 'Collection cloned successfully');
            res.json({
                success: true,
                uri: newListUri,
    ...
    router.get('/:listUri/items', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            const filteredRecords = response.data.records.filter((record) => record.value.list === listUri);
            // Batch fetch all media items to avoid N+1 queries
            const mediaItemIds = filteredRecords
                .filter((record) => record.value.mediaItemId)
                .map((record) => record.value.mediaItemId);
            const mediaItems = mediaItemIds.length > 0
                ? await ctx.db
    ...
    router.post('/quick-add', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            let defaultListUri = null;
            for (const record of listsResponse.data.records) {
                if (record.value.isDefault === true) {
            ...
            const existingItem = itemsResponse.data.records.find((record) => {
                const val = record.value;
                if (val.list !== defaultListUri)
                    return false;
                if (mediaItemId)
                    return val.mediaItemId === mediaItemId;
            ...
            if (existingItem) {
                ...
                const useritem = mediaItemId
                    ? useritemsResponse.data.records.find((r) => r.value.mediaItemId === mediaItemId)
                    : null;
                // If caller wants 'in-progress' and current is 'want', upgrade
                if (status === 'in-progress' &&
                    useritem &&
                    useritem.value.status === 'want') {
                    const existingVal = useritem.value;
                    await agent.api.com.atproto.repo.putRecord({
                        repo: agent.did,
                        collection: 'app.collectivesocial.feed.useritem',
                ...
                return res.json({
                    ...
                    userItemUri: existingItem.value.userItem || null,
                });
            }
            // Ensure a useritem record exists
            let userItemUri = null;
            const useritemsResponse = await agent.api.com.atproto.repo.listRecords({
                repo: agent.did,
                collection: 'app.collectivesocial.feed.useritem',
            });
            ...
            if (existingUseritem) {
                ...
                if (status === 'in-progress' &&
                    existingUseritem.value.status === 'want') {
                    const existingVal = existingUseritem.value;
                    await agent.api.com.atproto.repo.putRecord({
                        repo: agent.did,
                        collection: 'app.collectivesocial.feed.useritem',
            ...
            const existingInList = itemsResponse.data.records
                .filter((r) => r.value.list === defaultListUri)
                .map((r) => r.value.order || 0);
    ...
    router.post('/:listUri/items', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        }
        const { title, status, rating, review, notes, mediaType, creator, mediaItemId, recommendedBy, completedAt, } = req.body;
        if (!title) {
            return res.status(400).json({ error: 'Title is required' });
        }
        try {
            ...
            const existingItemsResponse = await agent.api.com.atproto.repo.listRecords({
                repo: agent.did,
                collection: 'app.collectivesocial.feed.listitem',
            });
            const existingItem = existingItemsResponse.data.records.find((record) => {
                const itemData = record.value;
                if (itemData.list !== listUri)
                    return false;
                if (mediaItemId) {
            ...
            const existingUseritem = mediaItemId
                ? useritemsResponse.data.records.find((record) => record.value.mediaItemId === mediaItemId)
                : null;
            if (existingUseritem) {
                userItemUri = existingUseritem.uri;
            }
                ...
                const useritemRecord = {
                    ...
                    completedAt: status === 'completed'
                        ...
                        : undefined,
                    recommendations: newRecommendations.length > 0 ? newRecommendations : undefined,
                    createdAt: now.toISOString(),
                    updatedAt: now.toISOString(),
                };
                const useritemResponse = await agent.api.com.atproto.repo.createRecord({
                    repo: agent.did,
                    collection: 'app.collectivesocial.feed.useritem',
                    record: useritemRecord,
                });
                ...
                if (review &&
                    ...
                    try {
                        ...
                        ctx.logger.error({ err }, 'Failed to create AT Protocol review record');
                    }
                    await ctx.db
                        .insertInto('reviews')
                        .values({
            ...
            const existingItemsInList = existingItemsResponse.data.records
                ...
                .map((record) => record.value.order || 0);
            const maxOrder = existingItemsInList.length > 0
    ...
    router.put('/:listUri/items/:itemUri', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            const itemRecord = itemsResponse.data.records.find((record) => record.uri === itemUri);
            if (!itemRecord) {
                return res.status(404).json({ error: 'Item not found' });
            }
            const currentData = itemRecord.value;
    ...
    router.put('/:listUri/reorder', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
        ...
        try {
            ...
            for (const itemUpdate of items) {
                const itemRecord = itemsResponse.data.records.find((record) => record.uri === itemUpdate.uri);
                if (!itemRecord)
                    continue;
                // Extract rkey
    ...
    router.delete('/:listUri/items/:itemUri', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const agent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!agent) {
            return res.status(401).json({ error: 'Not authenticated' });
    ...
    router.get('/public/:did', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'public, max-age=60');
        const { did } = req.params;
        // Try to get authenticated agent, otherwise create unauthenticated one
        let queryAgent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        if (!queryAgent) {
            // Create an unauthenticated agent for public queries
            queryAgent = new api_1.Agent({ service: 'https://bsky.social' });
        }
        try {
    ...
    router.get('/public/:did/in-progress', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'public, max-age=60');
        const { did } = req.params;
        // Try to get authenticated agent, otherwise create unauthenticated one
        let queryAgent = await (0, agent_1.getSessionAgent)(req, res, ctx);
        ...
        try {
            ...
            const collectionsResponse = await queryAgent.api.com.atproto.repo.listRecords({
                repo: did,
                collection: 'app.collectivesocial.feed.list',
            });
            ...
            const useritemsResponse = await queryAgent.api.com.atproto.repo.listRecords({
                repo: did,
                collection: 'app.collectivesocial.feed.useritem',
            });
            // Build a lookup map: mediaItemId → useritem data
            ...
            const inProgressItems = itemsResponse.data.records
                .filter((record) => {
                const listUri = record.value.list;
                if (!publicCollectionUris.includes(listUri))
                    return false;
            ...
            const uniqueInProgressItems = inProgressItems.filter((item) => {
                if (!item.mediaItemId || seen.has(item.mediaItemId))
                    return false;
