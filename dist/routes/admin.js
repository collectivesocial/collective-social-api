...
async function requireAdmin(req, res, ctx) {
...
const createRouter = (ctx) => {
    ...
    router.get('/users', (0, http_1.handler)(async (req, res) => {
        ...
        try {
            ...
            let userHandles = new Map();
            if (sessionDid) {
                try {
                    const oauthSession = await ctx.oauthClient.restore(sessionDid);
                    if (oauthSession) {
                        const adminAgent = new api_1.Agent(oauthSession);
                        // Fetch user handles
                        const userDids = users.map((user) => user.did);
                        userHandles = await (0, users_1.fetchUserHandles)(adminAgent, userDids, ctx.logger);
                    }
                }
                catch (err) {
                    ctx.logger.error({ err }, 'Failed to create admin agent for handle lookup');
                }
            }
    ...
    router.get('/media', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        if (!(await requireAdmin(req, res, ctx))) {
            return;
        }
    ...
    router.get('/share-links', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        if (!(await requireAdmin(req, res, ctx))) {
            return;
        }
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 20;
            const sortBy = req.query.sortBy || 'timesClicked'; // 'timesClicked' or 'createdAt'
            ...
            if (sortBy === 'timesClicked') {
                query = query.orderBy('share_links.timesClicked', order === 'asc' ? 'asc' : 'desc');
            }
            else {
                query = query.orderBy('share_links.createdAt', order === 'asc' ? 'asc' : 'desc');
            }
            // Apply pagination
            ...
            const sessionDid = await getSessionAgent(req, res, ctx);
            let userHandles = new Map();
            let adminAgentForCollections = null;
            if (sessionDid) {
                try {
                    const oauthSession = await ctx.oauthClient.restore(sessionDid);
                    if (oauthSession) {
                        ...
                        userHandles = await (0, users_1.fetchUserHandles)(adminAgent, uniqueUserDids, ctx.logger);
                    }
                }
                catch (err) {
                    ctx.logger.error({ err }, 'Failed to create admin agent for handle lookup');
            ...
            const linksByUser = shareLinks.reduce((acc, link) => {
            ...
            if (adminAgentForCollections) {
                for (const [userDid, userLinks] of Object.entries(linksByUser)) {
                    try {
                        ...
                        const listsResponse = await adminAgentForCollections.api.com.atproto.repo.listRecords({
                            repo: userDid,
                        ...
                        ctx.logger.error({ err, userDid }, 'Failed to fetch collection names for admin view');
                    }
                }
            }
            res.json({
    ...
    router.get('/check', (0, http_1.handler)(async (req, res) => {
        res.setHeader('cache-control', 'no-store');
        const did = await getSessionAgent(req, res, ctx);
        if (!did) {
