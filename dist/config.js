...
function requireSecureSecret(name) {
    const value = process.env[name];
    const insecureDefaults = ['', 'default', 'super-secret', 'secret', 'changeme'];
    if (!value || insecureDefaults.includes(value.toLowerCase())) {
        if (process.env.NODE_ENV === 'production') {
            throw new Error(`Environment variable ${name} must be set to a secure value in production`);
        }
        console.warn(`⚠️  WARNING: ${name} is using an insecure default. Set a proper secret before deploying.`);
...
exports.config = {
    ...
    clientUrl: process.env.CLIENT_URL ||
        (nodeEnv === 'production' ? process.env.SERVICE_URL : 'http://127.0.0.1:5173'),
    ...
    corsOrigin: process.env.CORS_ORIGIN || undefined, // e.g. https://app.collectivesocial.app
};
